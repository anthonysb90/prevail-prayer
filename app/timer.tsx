  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#0A0A0A" }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 32 }}>
        {/* Duration */}
        <SettingsLabel label="Duration" />
        <View style={{ flexDirection: "row", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
          {DURATIONS.map((d) => (
            <TouchableOpacity
              key={d.seconds}
              onPress={() => { setDuration(d.seconds); setRemaining(d.seconds); }}
              style={{
        ...
        {/* Ambient Sound */}
        <SettingsLabel label="Ambient Sound" />
        {TRACKS.map((t) => (
          <TouchableOpacity
            key={t.id}
          ...
          >
            <Text style={{ color: "#FFFFFF", fontFamily: "DMSans-Regular", fontSize: 15 }}>
              {t.label}
            </Text>
        ...
        {/* Bell Interval */}
        <SettingsLabel label="Bell Interval" />
        {BELL_OPTIONS.map((b) => (
          <TouchableOpacity
            key={b.id}
            onPress={() => setBellInterval(b.id)}
            style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: "#1A1A1A", borderRadius: 12, padding: 16, marginBottom: 8 }}
          >
            <Text style={{ color: "#FFFFFF", fontFamily: "DMSans-Regular", fontSize: 15 }}>
              {b.label}
...
function SettingsLabel({ label }: { label: string }) {
  return (
    <Text style={{ color: "#9A9A9A", fontFamily: "DMSans-Medium", fontSize: 12, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10, marginTop: 4 }}>
      {label}
    </Text>
  );
}
